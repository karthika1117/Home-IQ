...
// Service role key for privileged server-side operations (bypasses RLS securely).
...
app.post('/api/confirm-booking', async (req, res) => {
  try {
    ...
    // This uses the anon client so RLS filters to the current user automatically.
    ...
    if (requestRecord.status === 'Confirmed' || requestRecord.technician_id) {
      // Return the existing booking if one exists; use systemClient to bypass RLS
    ...
    // systemClient required: technicians RLS blocks customers from reading other rows
    ...
    // systemClient (service-role) sees ALL bookings across customers for this technician
    ...
    // ── 12. Create booking (service-role or anon, depending on RLS config) ────
    // Prefer service-role key on the server so RLS INSERT policies aren't needed
    // on the browser. Falls back to anon client if no service-role key is set.
    ...
    if (updateErr) {
      // Booking was created; log but don't fail the response
...
app.listen(PORT, () => {
  console.log(`HomeIQ backend server running on http://localhost:${PORT}`);
});
