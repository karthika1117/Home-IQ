  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data, error: fetchErr } = await supabase
        .from('bookings')
        .select('booking_id, service_category, service_date, start_time, end_time, status, technicians(name), area, address, customer_name')
            ...
            <tbody>
              {bookings.map(b => (
                <tr key={b.booking_id}>
                  ...
                  <td style={{ color: 'var(--color-text-secondary)' }}>{(b.start_time && b.end_time) ? `${b.start_time} - ${b.end_time}` : (b.start_time || '-')}</td>
